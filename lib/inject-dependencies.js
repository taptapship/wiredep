...
function replaceIncludes(file, fileType, returnType) {
  ...
  return function (match, startBlock, spacing, blockType, oldScripts, endBlock, offset, string) {
    ...
    //default value for detectOusideBlock is true, implying that if it's undefined
    if (fileType.detectOusideBlock || typeof(fileType.detectOusideBlock) === 'undefined') {
      //searching for entries outside the block

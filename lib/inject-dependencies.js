...
module.exports = function inject(config) {
  ...
  bowerPattern = config.get('bower-pattern');
  bowerPattern = regex.bower[bowerPattern] || bowerPattern;
  bowerPattern = bowerPattern || regex.bower.html;

  globalDependenciesSorted = config.get('global-dependencies-sorted');
  ignorePath = config.get('ignore-path');

  var file = config.get('html-file') || config.get('file');
  var html = config.get('html');

  returnType = /\r\n/.test(html) ? '\r\n' : '\n';

  // grab the html file and its contents, then drop our scripts in.
  fs.writeFileSync(file, html.replace(bowerPattern, injectScripts));

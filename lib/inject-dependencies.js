  ...
  return function (match, startBlock, spacing, blockType, oldScripts, endBlock, offset, string) {
    ...
    (string.substr(0, offset) + string.substr(offset + match.length)).
      ...
      replace(fileType.detect[blockType], function (match, reference) {
        ...
        //if detectOutsideBlock is true of not specified filesCaught will hold a list of files included manually outside the bower block
        //those file will be excluded from bower block
        if (fileType.detectOutsideBlock || typeof(fileType.detectOutsideBlock) === 'undefined') {

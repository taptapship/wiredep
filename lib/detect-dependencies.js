...
module.exports = function detect(config) {
  ...
  var prioritizedDependencies = {};
  _(config.get('file-types')).each(function (fileTypeConfig) {
    _(fileTypeConfig.detect).each(function (detectConfig, detectType) {
      if(!prioritizedDependencies[detectType]) {
        prioritizedDependencies[detectType] = prioritizeDependencies(config, '.' + detectType);
      }
    });
  });

  config.set('global-dependencies-sorted',
    filterExcludedDependencies(prioritizedDependencies, config.get('exclude')));

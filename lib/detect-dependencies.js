...
function detectDependencies(config) {
  ...
  var dependenciesFiles = config.get('detectable-file-types').
  ...
  $._.forOwn(config.get('extra-file-types'), function (fileTypes, key) {
    var files = [];
    if (fileTypes instanceof Array) {
      $._.each(fileTypes, function (fileType) {
        files = $._.concat(files, prioritizeDependencies(config, '.' + fileType));
      });
    } else {
      files = prioritizeDependencies(config, '.' + fileTypes);
    }

    if (!dependenciesFiles[key]) {
      dependenciesFiles[key] = files;
    }
  });

  config.set('global-dependencies-sorted',
    filterExcludedDependencies(dependenciesFiles, config.get('exclude')));

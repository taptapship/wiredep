 ...
 *
 * @param  {array} dependencies  array of dependencies to filter
 * @param  {array} patterns      array of patterns to match against
 * @return {array} items that don't match any of the patterns
 */
var excludeDependencies = function (dependencies, patterns) {
  return _.reject(dependencies, function(dependency){
...
module.exports = function detect(config) {
  ...
  if (config.get('exclude')) {
    var dependencies = config.get('global-dependencies-sorted');
    var filteredDependencies = {};

    var excludePatterns = config.get('exclude');

    for (var fileType in dependencies){
      filteredDependencies[fileType] = excludeDependencies(dependencies[fileType], excludePatterns);
    }
    config.set('global-dependencies-sorted', filteredDependencies);
  }

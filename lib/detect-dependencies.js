...
function findMainFiles(config, component, componentConfigFile) {
  ...
  if ($._.isString(componentConfigFile.main)) {
   ...
   filePaths.forEach(function (file) {
        var filePath = '';
        if (config.get('include-self') && component === config.get('bower.json').name) {
            filePath = $.path.join(config.get('cwd'));
        } else {
            filePath = $.path.join(config.get('bower-directory'), component);
        }
        $.glob(file, {sync: true,cwd: filePath }, function (er, filesFound) {
            filesFound.forEach(function(fileFound) {
                var newFile = $.path.join( filePath , fileFound);
                if (-1 === files.indexOf(newFile)) {
                    files.push(newFile);
                }
            });
        });

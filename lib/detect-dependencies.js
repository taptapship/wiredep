...
function detectDependencies(config) {
  ...
  if ($._.size(config.get('include-dependencies')) > 0) {
    //collect dependencies in predefined bower modules name if config.get('include-dependencies') is valid
    allDependencies = filterIncludedDependencies(allDependencies, config.get('include-dependencies'));
  }
...
/**
 * Includes dependencies that match any of the patterns.
 *
 * @param  {array} allDependencies  array of dependencies to filter
 * @param  {array} patterns         array of patterns to match
 * @return {array} items that match any of the bower module name patterns
 */
function filterIncludedDependencies(allDependencies, patterns) {
  return $._.reduce($._.keys(allDependencies), function(result, component) {
    if ($._.some(patterns, function(pattern) {
      return component.match(pattern);
    })) {
      result[component] = allDependencies[component];
    }
    return result;
  }, {});
}
 ...
 * @return {array} items that don't match any of the file name patterns
 */

 ...
 * @param {boolean} fromMainConfig  whether this component is the top-level bower.json
 * @return {array} the array of paths to the component's primary file(s)
 */
var findMainFiles = function (config, component, componentConfigFile, fromMainConfig) {
  var filePaths = [];
  var file;
  ...
  return filePaths.map(function (file) {
    if (fromMainConfig){
      return path.join(config.get('main-file-dir'), file);
    } else {
...
/**
 * Add main files listed in the top-level bower.json in the injected files list.
 *
 * @param  {object} config   the global configuration object
 * @return {undefined}
 */
var includeCurrentMainFiles = function(config) {
  var mainConfigFile = config.get('bower.json');
  var warnings = config.get('warnings');
  var dep = {
      main: '',
      type: '',
  ...
  var mains = findMainFiles(config, mainConfigFile.name, mainConfigFile, true);
  var fileTypes = _.chain(mains).map(path.extname).unique().value();
  var name = mainConfigFile.name;
  ...
  if (dep.main.length === 0) {
    warnings.push('The main files of the current project were not injected into your file.');
    warnings.push('Please take a look at your bower.json and ensure that you have specified them.');
    config.set('warnings', warnings);
    return;
  }
  config.get('global-dependencies').set(name, dep);
};
...
module.exports = function detect(config) {
  ...
  if (config.get('include-mains')) {
    includeCurrentMainFiles(config);
  }

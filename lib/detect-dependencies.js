...
var findMainFiles = function (config, component, componentConfigFile) {
  ...
  if (_.isString(componentConfigFile.main)) {
    ...
    // check for file named like the component
    file = path.join(config.get('bower-directory'), component, componentConfigFile.name + '.js');
    if (fs.existsSync(file)) {
      filePaths = [componentConfigFile.name + '.js'];
    } else {
      // if component named file does not exist check for distDirs - default value is ['dist']
      // also, it can be an object like that {'jquery': ['dist']}
      distDirsConfig = config.get('dist-dirs');

      // if an object - use the corresponding array of dist dir names
      if (_.isObject(distDirsConfig) && component in distDirsConfig) {
        distDirsConfig = distDirsConfig[component];
      }

      if (distDirsConfig.length) {
        distFilePaths = [];

        _.each(distDirsConfig, function (distDir) {
          distDirName = path.join(config.get('bower-directory'), component, distDir);
          if (fs.lstatSync(distDirName).isDirectory()) {
            file = path.join(config.get('bower-directory'), component, distDir, componentConfigFile.name + '.js');

            if (fs.existsSync(file)) {
              distFilePaths.push(path.join(distDir, componentConfigFile.name + '.js'));
            }
          }
        });

        if (distFilePaths.length) {
          filePaths = [distFilePaths[0]];
        }
      }
    }
  }

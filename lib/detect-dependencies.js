...
function detectDependencies(config) {
  ...
  if ($._.size(config.get('include-dependencies')) > 0) {
    ...
    allDependencies = filterIncludedDependencies(allDependencies, config);
  }
 ...
 */
function filterIncludedDependencies(allDependencies, config) {
  return $._.reduce($._.keys(allDependencies), function(result, component) {
    if ($._.some(config.get('include-dependencies'), function(pattern) {
      return $.path.join(config.get('bower-directory'), component).match(pattern);
    })) {
